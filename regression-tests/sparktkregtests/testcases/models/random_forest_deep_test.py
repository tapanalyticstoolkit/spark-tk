...
class RandomForestDeep(sparktk_test.SparkTKTestCase):
    ...
    def setUp(self):
        ...
        schema = [("feat"+str(i), int) for i in range(9)] + [("class", int)]
    ...
    def test_deep_regression_tree(self):
        ...
        schema = ["feat"+str(i) for i in range(9)]
        model32 = self.context.models.regression.random_forest_regressor.train(
            self.frame, schema, "class", max_depth=32, seed=0)

        model34 = self.context.models.regression.random_forest_regressor.train(
            self.frame, schema, "class", max_depth=34, seed=0)
        result32 = model32.test(self.frame)
        result34 = model34.test(self.frame)
        ...
        self.assertGreater(
            result32.mean_squared_error, result34.mean_squared_error)
